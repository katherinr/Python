...
def process_sprite_group(canvas, sprite_group):
    for smth in sprite_group:
        smth.draw(canvas)
        smth.update()
        if smth.age>=smth.lifespan:
           sprite_group.remove(smth)
...
def group_group_collide(group_one, group_two):
    ...
    counts=0
    for element in list(group_two):
        check=group_collide(group_one,element)
        if check:
           group_two.discard(element)
           pose=element.get_position()
